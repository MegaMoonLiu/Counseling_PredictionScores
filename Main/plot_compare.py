...
def plot_grouped_bars(pivoted: pd.DataFrame, metric: str, out_path: Path) -> None:
    ...
    preferred = [c for c in ["EGPS (Ours)", "Llama-3.1"] if c in pivoted.columns]
    ...
    ax.set_title(f"EGPS (Ours) vs Llama-3.1 — {metric}")
...
def main() -> None:
    ...
    for metric in METRICS:
        ...
        out_file = out_dir / f"EGPS_vs_Llama-3.1_{_safe_name(metric)}_{stamp}.png"
        plot_grouped_bars(pv, metric, out_file)

    print("Done. Generated:")
    for metric in METRICS:
        print(f"- EGPS_vs_Llama-3.1_{_safe_name(metric)}_{stamp}.png")

    # 计算最大差值
    pv = df.pivot_table(index="Dim.", columns="Model", values=METRICS, aggfunc="mean")

    for m in METRICS:
        if (m, "EGPS") not in pv.columns or (m, "Llama-3.1") not in pv.columns:
            raise ValueError(f"Pivot missing EGPS/Llama-3.1 for metric: {m}")

    diff = pd.DataFrame(index=pv.index)
    abs_diff = pd.DataFrame(index=pv.index)

    for m in METRICS:
        diff[m] = pv[(m, "EGPS")] - pv[(m, "Llama-3.1")]
    ...
    detail = pd.concat(
        {
            "EGPS": pv.xs("CoT", axis=1, level=1),
            "Llama-3.1": pv.xs("Paper", axis=1, level=1),
            "Diff(EGPS-Llama-3.1)": diff,
            "AbsDiff": abs_diff,
            "SumAbsDiff": combined,
        },
        axis=1,
